...
class Data_model extends CI_Model {
        ...
        public function typecase()// หมวดเคส
        {
                $query = $this->db->get('tbl_type_case');
                return $query->result();
        }
...
//query count by status
    ...
    //query count by case_type
    public function show_type_case()
    {
    ...
    public function show_type_company()
    {
        $this->db->where('is_approve',1);
...
//query count by status
    ...
    public function countcase_UserIT()
    {
            $this->db->select('tech_id,tech_name,COUNT( id) AS totolcase');
            $this->db->from('tbl_case');
            $this->db->group_by('tech_name');
            $query = $this->db->get();
            return $query->result();
            /* if($query->num_rows() > 0){
                    $data = $query->row();
